        ...
        <div class="w-2/4 m-auto mt-9 justify-center">
            ...
            <div class="slotWrap border rounded-md shadow-md">
                <div class="header flex text-gray-800 bg-gray-200 px-6 py-2">
                    ...
                    <div class="px-2 py-3 flex-auto w-1/6">{{__('HE')}}</div>
                    <div class="px-2 py-3 flex-auto w-1/4">{{__('Provider')}}</div>
                    <div class="px-2 py-3 flex-auto w-1/4">{{__('Action')}}</div>
                </div>
                <div class="body">
                    @foreach($slots as $slot)
                        <div class="singleslot flex px-6 py-2">
                            <div class="px-2 py-3 flex-auto w-1/4">{{ $slot->name }}</div>
                            <div class="px-2 py-3 flex-auto w-1/6">{{ $slot->he }}</div>
                            <div class="px-2 py-3 flex-auto w-1/4">{{ $slot->provider }}</div>
                            <div class="px-2 py-3 flex gap-4 flex-auto w-1/4">
                                <a href="{{route('slot.edit', $slot)}}" class="px-3 py-3 border rounded hover:text-gray-50 hover:bg-gray-800">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                        <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                                        <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                                    </svg>
                                </a>
                                <form method="post" action="{{ route('slot.destroy', $slot) }}">
                                    @method('DELETE')
                                    @csrf
