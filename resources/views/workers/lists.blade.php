        ...
        <div class="w-2/4 m-auto mt-9 justify-center">
            ...
            <div class="slotWrap border rounded-md shadow-md">
                <div class="header flex text-gray-800 bg-gray-200 px-6 py-2">
                    ...
                    <div class="px-2 py-3 flex-auto w-1/4">{{ __('Assigned User') }}</div>
                    <div class="px-2 py-3 flex-auto w-1/4">{{ __('Owed') }}</div>
                    <div class="px-2 py-3 flex-auto w-1/4">{{__('Action')}}</div>
                </div>
                <div class="body">
                    @foreach($workers as $k => $worker)
                        <div class="singleslot flex px-6 py-2">
                            <div class="px-2 py-3 flex-auto w-1/4">{{ $worker->name }}</div>
                            <div class="px-2 py-3 flex-auto w-1/4">{{ !empty($worker->display_name) ? $worker->display_name : $worker->user_name }}</div>
                            <div class="px-2 py-3 flex-auto w-1/4">{{ $worker->owed }}</div>
